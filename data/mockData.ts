import { PO, PSO, Course, ArticulationMatrix, Student, AttainmentData, User, Department, CO } from '../types';
...
const generateCOs = (courseId: string, courseName: string): CO[] => {
...
export const courses: Course[] = courseDefinitions.map((def, index) => ({
    ...def,
...
export const articulationMatrix: ArticulationMatrix = {};
...
export const coAttainmentData: AttainmentData[] = courses[0].cos.map((co, index) => ({
    name: co.id.split('.')[1],
    target: 60,
    attained: 55 + index * 5 - Math.random() * 10,
}));

export const poAttainmentData: AttainmentData[] = pos.map((po, index) => ({
    name: po.id,
    target: 70,
...
export const coursePerformanceData: (AttainmentData & { courseId: string })[] = courses.map(course => ({
